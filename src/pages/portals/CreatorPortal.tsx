...
const CreatorPortal = () => {
  ...
  useEffect(() => {
    ...
    void checkAutoCloseOffers(offers, bids);
          ...
          {availableOffers.length === 0 ? (
            ...
            <div className="space-y-4">
              {availableOffers.map((offer) => {
                ...
                // One-time profile gate. Map blurs until creator saves both
                // phone + BaridiMob to their profile. Click → jumps to profile.
                const mapUnlocked = !!(myUserDoc?.phone && myUserDoc?.bariMobAccount);
                ...
                return (
                  <div key={offer.id} className="glass rounded-2xl p-5 hover:border-accent/30 transition-smooth">
                    ...
                    {/* Precise location map — blurred until the creator saves
                        their phone + BaridiMob (one-time profile gate). */}
                    {offer.clientWilaya && (
                      <div className="relative mt-3 mb-3 rounded-xl overflow-hidden border border-border/40">
                        <OfferMap
                          wilaya={offer.clientWilaya}
                          lat={offer.locationLat}
                          lng={offer.locationLng}
                          zoom={typeof offer.locationLat === "number" ? 13 : undefined}
                          height={160}
                        />
                        {!mapUnlocked && (
                          <button
                            ...
                            className="absolute inset-0 backdrop-blur-md bg-background/60 flex flex-col items-center justify-center gap-2 px-4 text-center cursor-pointer hover:bg-background/70 transition-smooth"
                          >
                            ...
                            <div className="text-sm font-semibold">
                              {lang === "ar"
                                ? "أضف رقمك وحسابك في بريدي موب لرؤية الموقع"
                                : lang === "fr"
                                ? "Ajoutez votre numéro et compte BaridiMob pour voir le lieu"
                                : "Add your phone + BaridiMob to see the location"}
                            </div>
                            <span className="text-xs text-accent underline">
                              {lang === "ar" ? "إكمال الملف الشخصي" : lang === "fr" ? "Compléter le profil" : "Complete profile"}
                            </span>
                          </button>
                        )}
                      </div>
            ...
            myBids.map((bid) => {
              ...
              return (
                <div key={bid.id} className="glass rounded-2xl p-5">
                  ...
                  {/* Auto-shared client contact (revealed at acceptBid). */}
                  {(bid.status === "accepted" || bid.status === "delivered") && bid.clientPhone && (
                    <div className="mt-3 pt-3 border-t border-border">
                      <p className="text-xs font-semibold text-emerald-400 mb-2 flex items-center gap-1.5">
                        <Phone className="w-3.5 h-3.5" />
                        {lang === "ar" ? "بيانات العميل" : lang === "fr" ? "Contact du client" : "Client contact"}
                      </p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                        {bid.clientName && (
                          <div className="rounded-md border border-border/40 px-3 py-2">
                            <div className="text-muted-foreground text-[10px] mb-0.5">{lang === "ar" ? "الاسم" : "Name"}</div>
                            <div className="font-medium">{bid.clientName}</div>
                          </div>
                        )}
                        <a
                          href={`tel:${bid.clientPhone}`}
                          className="rounded-md border border-border/40 px-3 py-2 hover:border-emerald-400/40 transition-smooth"
                        >
                          <div className="text-muted-foreground text-[10px] mb-0.5">{lang === "ar" ? "اتصال" : "Call"}</div>
                          <div className="font-medium" dir="ltr">{bid.clientPhone}</div>
                        </a>
                        <a
                          href={`https://wa.me/${bid.clientPhone.replace(/[^\d]/g, "")}`}
                          target="_blank"
                          rel="noreferrer"
                          className="rounded-md border border-border/40 px-3 py-2 hover:border-emerald-400/40 transition-smooth"
                        >
                          <div className="text-muted-foreground text-[10px] mb-0.5">WhatsApp</div>
                          <div className="font-medium text-emerald-400">{lang === "ar" ? "محادثة" : "Open chat"}</div>
                        </a>
                        {offer?.clientWilaya && (
                          <div className="rounded-md border border-border/40 px-3 py-2">
