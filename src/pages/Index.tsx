...
const Index = () => {
  ...
  const heroPrefix =
    lang === "ar"
      ...
      : lang === "fr"
      ? "La première plateforme audiovisuelle algérienne réunissant clients et freelances avec"
    ...
    <div className="min-h-screen">
      ...
      <section className="relative pt-32 pb-24 px-4 sm:px-6 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto text-center">
          ...
          {/* Animated tagline */}
          <div className="text-base sm:text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto mb-10 leading-relaxed">
            <AnimatedHeroText
              prefix={heroPrefix}
              words={animatedWords}
          ...
          {/* Stats — clients & creators from Firestore */}
          <div className="grid grid-cols-2 gap-4 max-w-sm mx-auto mt-14">
            <div className="glass rounded-2xl p-4 text-center">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                {lang === "ar" ? "عميل مسجّل" : lang === "fr" ? "Clients inscrits" : "Registered clients"}
              </div>
            </div>
            <div className="glass rounded-2xl p-4 text-center">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                {lang === "ar" ? "عامل حر" : lang === "fr" ? "Freelances" : "Freelancers"}
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="offers" className="px-4 sm:px-6 py-20 max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground mt-3 max-w-xl mx-auto text-sm">{t("servicesSub")}</p>
        </div>

        {/* Simple CSS grid — no JS animations, no lag */}
          ...
          {OFFERS.map((o) => {
            ...
            return (
              <Link
                key={o.slug}
                to={`/services/${o.slug}`}
                className="block group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent rounded-[18px]"
              >
                <GlowCard variant={o.accent === "gold" ? "gold" : "royal"} className="group-hover:-translate-y-0.5 transition-transform duration-200">
                  ...
                  <div className="flex flex-col flex-1 p-4">
                    ...
                    <div className={`mt-3 flex items-center gap-1 text-xs font-medium ${o.accent === "gold" ? "text-accent" : "text-primary-foreground/70 group-hover:text-white"} transition-colors`}>
                      {t("discover")} <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform" />
                    </div>
                  </div>
                </GlowCard>
              </Link>
      ...
      <section id="university" className="px-4 sm:px-6 py-16 max-w-6xl mx-auto">
        <div className="relative glass rounded-3xl overflow-hidden">
          <div className="absolute inset-0">
            <img src={ub.image} alt="" loading="lazy" className="w-full h-full object-cover opacity-12" />
            ...
            <div className="flex flex-wrap gap-3">
              <Button asChild variant="royal" size="lg">
                <Link to="/auth/signup?role=client">
                  {lang === "ar" ? "طلب العرض الجامعي" : lang === "fr" ? "Demander l'offre" : "Request offer"} <ArrowRight className="ms-2 w-4 h-4" />
                </Link>
              </Button>
      ...
      <section id="about" className="px-4 sm:px-6 py-16 max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-4">
          <div className="glass rounded-3xl p-7 sm:p-10 relative overflow-hidden">
            ...
            <span className="text-xs uppercase tracking-widest text-accent">{t("forBrands")}</span>
            <h3 className="font-serif text-2xl sm:text-4xl font-bold mt-3 mb-4">{t("briefUs")}</h3>
            <p className="text-muted-foreground mb-7 text-sm">{t("brandSub")}</p>
          ...
          <div className="glass rounded-3xl p-7 sm:p-10 relative overflow-hidden">
            ...
            <span className="text-xs uppercase tracking-widest text-accent">{t("forCreatorsSection")}</span>
            <h3 className="font-serif text-2xl sm:text-4xl font-bold mt-3 mb-4">{t("bringPortfolio")}</h3>
            <p className="text-muted-foreground mb-7 text-sm">{t("creatorSub")}</p>
