    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 px-6 pt-28 pb-12 max-w-5xl mx-auto w-full">
        <div className="text-center mb-12 animate-fade-in">
          <span className="text-xs uppercase tracking-[0.3em] text-accent">Join North Pixel</span>
          <h1 className="font-serif text-4xl md:text-5xl font-bold mt-3">Who are you?</h1>
          <p className="text-muted-foreground mt-3">Pick the path that fits you. You can always switch later.</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <Link to="/auth/signup/client" className="group glass rounded-3xl p-8 relative overflow-hidden transition-smooth hover:-translate-y-1 hover:border-primary-glow/50 animate-fade-in">
            ...
            <div className="relative">
              ...
              <h2 className="font-serif text-2xl font-bold mb-2">I'm a Client</h2>
              <p className="text-muted-foreground mb-6">{t("brandSub")}</p>
              <span className="inline-flex items-center gap-1 text-accent font-medium">{t("openClient")} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition" /></span>
            </div>
          </Link>
          ...
          <Link to="/auth/signup/creator" className="group glass rounded-3xl p-8 relative overflow-hidden transition-smooth hover:-translate-y-1 hover:border-accent/50 animate-fade-in">
            ...
            <div className="relative">
              ...
              <h2 className="font-serif text-2xl font-bold mb-2">I'm a Creator</h2>
              <p className="text-muted-foreground mb-6">{t("creatorSub")}</p>
              <span className="inline-flex items-center gap-1 text-accent font-medium">{t("applyCreator")} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition" /></span>
            </div>
          </Link>
        </div>

        <p className="text-center text-sm text-muted-foreground mt-10">
          Already have an account? <Link to="/auth/login" className="text-accent">{t("login")}</Link>
        </p>
      </main>
      <SiteFooter />
    </div>
